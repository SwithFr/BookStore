        ...
        <?php for ($i = 'a'; $i <= 'z'; $i++): ?>
            <li class="filter__item">
                <?php if (in_array(ucfirst($i), $data['letters'])): ?>
                    <a href="<?= Html::url('index', 'editor', ['letter' => $i]); ?>"
                       class="dispo-letter<?= ($data['letter'] == $i) ? ' filter--active' : ''; ?>"><?= ucfirst($i); ?></a>
                <?php else: ?>
                    <?= ucfirst($i); ?>
                <?php endif; ?>
                <?php if ($i == 'z') {
    ...
    <ul class="editors__list">
        <?php foreach ($data['editors'] as $editor): ?>
            <li class="section__block editors__list__item">
                ...
                <h2 class="editor__title"><a
                        href="<?= Html::url('view', 'editor', ['id' => $editor->id]); ?>"><?= $editor->name; ?></a></h2>
                <span class="editor__infos"><span
                        class="nb"><?= $editor->book_count; ?></span> livres sur ce site</span>
