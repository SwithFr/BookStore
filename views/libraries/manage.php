...
<div class="btn__group">
    ...
    <?php if (!$data['hasLocation']): ?>
        <a class="btn alert alert--warning" href="<?= Html::url('add', 'editor'); ?>">Vous n'avez pas encore ajouté
            d'emplacement, ajoutez en un !</a>
    <?php else: ?>
        <a class="btn btn--add btn--inline"
           href="<?= Html::url('add', 'location', ['library' => $data['library']->id]); ?>">Ajouter un emplacement</a>
    <?php endif; ?>
</div>
    ...
    <table class="books__list">
        ...
        <?php foreach ($data['books'] as $book): ?>
            <tr class="books__list__item">
                ...
                <td class="actions">
                    <a href="<?= Html::url('edit', 'book', ['id' => $book->id, 'library' => $data['library']->id]); ?>">Edit<i
                            class="icon-pencil"></i></a>
                    <a href="<?= Html::url('delete', 'book', ['id' => $book->id]); ?>" class="admin--delete">Suppr<i
